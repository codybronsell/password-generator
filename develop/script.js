...
console.info(inlow)
if(inlow){
    finalList = finalList.concat(lowlist);
}
if(innumeric){
    finalList = finalList.concat(numlist);
}
if(inupper){
    finalList = finalList.concat(upperlist);
}
if(inspecial){
    finalList = finalList.concat(speclist);
}

for (var i = finalList.length - 1; i > 0; i--) {
    var j = Math.floor(Math.random() * (i + 1));
    var temp = finalList[i];
    finalList[i] = finalList[j];
    finalList[j] = temp;
}
return finalList.slice(0, passsize).toString().replaceAll(',', '');
